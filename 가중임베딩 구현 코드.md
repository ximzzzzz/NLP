...
아래 코드와 설명은 이기창님의 `한국어 임베딩` 책에 나온 코드를 참조했습니다.
...
```python
Model = CBoWModel(train_corpus_path, embedding_path, output_path, embedding_corpus_path, embedding_name, True)


words, vecs = Model.load_word_embeddings(embedding_path, embedding_name)

words_count, total_count = Model.compute_word_frequency(embedding_corpus_path)

dictionary = {}
a = 0.0001
with open( embedding_path + "-weighted", "w") as f3:
    for word, vec in zip(words, vecs):
        if word in words_count.keys():
...
```python
Model.embeddings = dictionary
model_full_fname = output_path + "-weighted"
```

```python
#train_model
model = {"vectors": [], "labels": [], "sentences": []}
train_data = Model.load_or_tokenize_corpus(train_corpus_path)
with open(model_full_fname, "w") as f:
    for sentence, tokens, label in train_data:
        tokens = Model.tokenizer.morphs(sentence)
